        ...
        <div class="container">
            <div class="add_content">
                <div class="add_content_up">
                    <h1>Редактировать - {{$kurs->name}}</h1>
                </div>
                <div class="modal">
                    <form action="/edit_kurs/edit_kurs/{{$kurs->id}}"  method="post" id="edit_kurs">
                        <div class="modal_add_inner">
                            <div class="modal_add_left">
                                <div>
                                    ...
                                    <div class="animate_input">
                                        <div class="form">
                                            <input type="text" name="name" id="name" placeholder="" value="@if(old('name')){{old('name')}}@else{{$kurs->name}}@endif">
                                ...
                                <div>
                                    ...
                                    <div class="form_textarea">
                                        <textarea name="descript" id="textarea" rows="3" placeholder="описание курса">@if(old('descript')){{old('descript')}}@else{{$kurs->descript}}@endif</textarea>
                                        @error('descript')
                                        <p class="error red">{{$message}}</p>
                                        @enderror
                                    </div>
                            ...
                            <input type="text" form="edit_kurs" name="lessons" id="lessons_input" value="@if(old('lessons')){{old('lessons')}}@else{{json_decode($kurs->lessons)}}@endif" required style="display:none; position:static">
                            ...
                            <div class="modal_add_right">
                                ...
                                @php
                                $lessons_free = Lesson::where('modul', 0)->get();
                                ...
                                <div id="choose_lessons">
                                    ...
                                    <div class="lesson selected_lesson" data-id="{{$kurs_lesson->id}}"><span class="checkbox checkbox_active">✔</span>
                                        <p class="text3">{{$kurs_lesson->name}}</p>
                                    </div>
                                    @endforeach
                                    @foreach ( $lessons_free as $lesson)
                ...
                <div class="add_content_down">
                    <button type="submit" form="edit_kurs" class="text2 button black_btn" disabled id="third_add_step">Редактировать курс</button>
                </div>
